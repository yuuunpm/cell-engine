...
const PersistLayer = (() => {
  ...
  async function saveCell(cellData) { return _transaction('cells', 'readwrite', (store) => store.put(cellData)); }
  async function saveCells(cellsArray) {
    return new Promise((resolve, reject) => {
      if (!db) { reject(new Error('数据库未初始化')); return; }
      const tx = db.transaction('cells', 'readwrite');
      const store = tx.objectStore('cells');
      for (const cell of cellsArray) { store.put(cell); }
      tx.oncomplete = () => resolve();
      tx.onerror = () => reject(tx.error);
    });
  }
  async function loadCell(id) { return _transaction('cells', 'readonly', (store) => store.get(id)); }
  async function loadAllCells() { return _transaction('cells', 'readonly', (store) => store.getAll()); }
  async function deleteCell(id) { return _transaction('cells', 'readwrite', (store) => store.delete(id)); }
  ...
  async function saveWorld(worldData) { return _transaction('worlds', 'readwrite', (store) => store.put(worldData)); }
  async function loadWorld(id) { return _transaction('worlds', 'readonly', (store) => store.get(id)); }
  async function loadLatestWorld() {
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = (e) => {
        ...
        if (cursor) { resolve(cursor.value); } else { resolve(null); }
      };
      request.onerror = () => reject(request.error);
    });
  }
  async function saveSetting(key, value) { return _transaction('settings', 'readwrite', (store) => store.put({ key, value })); }
  async function loadSetting(key) {
    const result = await _transaction('settings', 'readonly', (store) => store.get(key));
    return result ? result.value : null;
  }
  async function loadAllSettings() { return _transaction('settings', 'readonly', (store) => store.getAll()); }
  async function saveFile(filePath, content, cellId) {
    if (!filePath || typeof content !== 'string') return;
    return _transaction('files', 'readwrite', (store) =>
      store.put({ path: filePath, content, cellId: cellId || null, updatedAt: Date.now() })
    );
  }
  async function loadFile(filePath) {
    if (!filePath) return null;
    const record = await _transaction('files', 'readonly', (store) => store.get(filePath));
    return record ? record.content : null;
  }
  async function deleteFile(filePath) {
    if (!filePath) return;
    return _transaction('files', 'readwrite', (store) => store.delete(filePath));
  }
  ...
  async function saveApiKey(providerId, apiKey) {
    const encoded = btoa(apiKey.split('').map((c, i) => String.fromCharCode(c.charCodeAt(0) ^ (i % 256))).join(''));
  ...
  async function loadApiKey(providerId) {
    ...
    try {
      ...
      return decoded.split('').map((c, i) => String.fromCharCode(c.charCodeAt(0) ^ (i % 256))).join('');
    } catch (e) { return null; }
  }
  function bindCellCore(cellCore) { _cellCoreRef = cellCore; }
  function markDirty(cellId) { _dirtyCells.add(cellId); }
  function _startAutoSave(getCellData) {
    ...
    _autoSaveTimer = setInterval(async () => {
      ...
      } catch (e) { console.error('[PersistLayer] 自动保存失败:', e); }
    }, AUTO_SAVE_INTERVAL);
  }
  ...
  async function saveFullSnapshot(getAllCellData, cameraState) {
    try {
      ...
      const worldData = { id: 'world_' + Date.now(), timestamp: Date.now(), cameraState, cellIds: cells.map(c => c.id) };
      await saveWorld(worldData);
      _dirtyCells.clear();
      console.log('[PersistLayer] 全量快照保存完成: ' + cells.length + ' 个基圆');
    } catch (e) { console.error('[PersistLayer] 全量快照保存失败:', e); }
  }
  ...
  function getDefaultWorldCells() {
    ...
    const plantData = [
      { id: 'plant_1', key: 'setaria_viridis', name: '狗尾草', x: -120, y: -80, color: '#6ba84f', size: 20 },
      { id: 'plant_2', key: 'setaria_viridis', name: '狗尾草', x: 130, y: -90, color: '#6ba84f', size: 20 },
      { id: 'plant_3', key: 'setaria_viridis', name: '狗尾草', x: 0, y: -160, color: '#6ba84f', size: 22 },
      { id: 'plant_4', key: 'taraxacum_officinale', name: '蒲公英', x: -150, y: 80, color: '#5fa03e', size: 22 },
      { id: 'plant_5', key: 'taraxacum_officinale', name: '蒲公英', x: 160, y: 90, color: '#5fa03e', size: 22 }
    ];
    for (let i = 0; i < plantData.length; i++) {
      const p = plantData[i];
      const isSetaria = p.key === 'setaria_viridis';
      cells.push({
        id: p.id,
        name: p.name,
        kind: 'plant',
        ...
        parentId: null,
        childrenIds: [],
        description: p.name + '：工蚁的重要食物来源。',
        code: isSetaria ? '// 植物基圆代码\nif (!api.getProperty("initialized")) {\n  api.setProperty("initialized", true);\n  api.setProperty("name", "狗尾草");\n  api.setProperty("species", "setaria_viridis");\n  api.setKind("plant");\n  api.setProperty("type", "grass");\n  api.setColor("#6ba84f");\n  api.setRadius(20);\n  api.setProperty("seedEnergy", 10);\n  api.setProperty("seedsRemaining", 3);\n  api.setProperty("growth", 0);\n  api.setProperty("preferred", 0.9);\n  api.setProperty("nutrients", 5);\n}\n\nif (api.getFrame() % 600 === 0) {\n  const g = Math.min(1, (api.getProperty("growth") || 0) + 0.03);\n  api.setProperty("growth", g);\n  api.setRadius(20 * (0.6 + g * 0.4));\n}\n\nif (api.getFrame() % 1200 === 0 && (api.getProperty("seedsRemaining") || 0) > 0) {\n  api.setProperty("seedsRemaining", (api.getProperty("seedsRemaining") || 0) - 1);\n  var sx = api.getX() + (Math.random() - 0.5) * 30;\n  var sy = api.getY() + (Math.random() - 0.5) * 30;\n  api.createCell({\n    kind: "plant",\n    x: sx,\n    y: sy,\n    name: "草籽",\n    color: "#c8b050",\n    radius: 3,\n    attributes: {\n      seedEnergy: 10,\n      type: "seed",\n      species: "setaria_viridis"\n    }\n  });\n}' : '// 植物基圆代码\nif (!api.getProperty("initialized")) {\n  api.setProperty("initialized", true);\n  api.setProperty("name", "蒲公英");\n  api.setProperty("species", "taraxacum_officinale");\n  api.setKind("plant");\n  api.setProperty("type", "grass");\n  api.setColor("#5fa03e");\n  api.setRadius(22);\n  api.setProperty("seedEnergy", 6);\n  api.setProperty("seedsRemaining", 2);\n  api.setProperty("growth", 0);\n  api.setProperty("preferred", 0.9);\n  api.setProperty("nutrients", 5);\n}\n\nif (api.getFrame() % 600 === 0) {\n  const g = Math.min(1, (api.getProperty("growth") || 0) + 0.03);\n  api.setProperty("growth", g);\n  api.setRadius(22 * (0.6 + g * 0.4));\n}\n\nif (api.getFrame() % 1200 === 0 && (api.getProperty("seedsRemaining") || 0) > 0) {\n  api.setProperty("seedsRemaining", (api.getProperty("seedsRemaining") || 0) - 1);\n  var sx = api.getX() + (Math.random() - 0.5) * 30;\n  var sy = api.getY() + (Math.random() - 0.5) * 30;\n  api.createCell({\n    kind: "plant",\n    x: sx,\n    y: sy,\n    name: "草籽",\n    color: "#c8b050",\n    radius: 3,\n    attributes: {\n      seedEnergy: 6,\n      type: "seed",\n      species: "taraxacum_officinale"\n    }\n  });\n}',
        compiledFn: null,
        attributes: { species: p.key, type: 'grass', seedEnergy: isSetaria ? 10 : 6, seedsRemaining: isSetaria ? 3 : 2, growth: 0, preferred: 0.9, nutrients: 5 },
