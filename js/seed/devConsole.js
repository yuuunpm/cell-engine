...
const DevConsole = (() => {
  ...
  async function _aiCommand(rawText) {
    ...
    const systemPrompt =
      ...
      '  4. 创建蚁后：优先使用 create_queen 工具（自动加载完整的探索→建巢→产蚁行为），不要用 create_ant + role=queen。\n' +
      '  5. 创建蚂蚁时，role 可选值: worker(工蚁)、soldier(兵蚁)、queen(蚁后)。\n' +
      '  6. 可用场景/地图关键词: 随机、草原、雨林、森林、沙漠、雨林、热带雨林。\n' +
      '  7. 用户请求"创建蚁后"/"生成蚁后"/"放一只蚁后"等时，直接调用 create_queen()，无需先查物种列表。\n' +
      '  8. 用户请求"建一张沙漠地图"/"生成草原场景"/"创建雨林"等时，直接调用 create_map_scene({description: "..."})，无需先查地图预设。\n\n' +
